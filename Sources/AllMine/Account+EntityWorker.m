...
- (double) balanceToDate:(NSDate*)dtTo forItem:(int)reportItem{
    if ([dtTo compare:[[NSDate today] dayNext]]==NSOrderedDescending && reportItem != 1) {
        return 0;
    }else if (reportItem == 1){
        if([dtTo compare:[[NSDate today] weekNext]] == NSOrderedDescending)
            return 0;
    }
    NSPredicate *predicate = [NSPredicate predicateWithFormat:@"finished == YES AND date <= %@", dtTo];
    NSNumber* sum1 = [[self.payments filteredSetUsingPredicate:predicate] valueForKeyPath:@"@sum.sumMain"];
    NSNumber* sum2 = [[self.paymentsAsCategory filteredSetUsingPredicate:predicate] valueForKeyPath:@"@sum.sumMain"];
    return [sum1 doubleValue] - [sum2 doubleValue];
}
...
- (double) creditLimitWithCurrency:(Currency*)currency{
    ...
    for (Payment* p in paymentsFinished) {
        if(self.type == [NSNumber numberWithInt:AccTypeCardCredit] && p.hidden == [NSNumber numberWithInt:0])
...
- (double) balanceWithCurrency:(Currency*)currency{
    ...
    NSSet* paymentsFinished = [self.payments filteredSetUsingPredicate:predicate];
    for (Payment* p in paymentsFinished) {
        if(self.type == [NSNumber numberWithInt:AccTypeCardCredit]){
            if(p.hidden == [NSNumber numberWithInt:1]){
                sum += ([p.sum doubleValue] * [p.currency rateValueWithDate:p.date mainCurrency:currency]);
            }
        }else
            sum += ([p.sum doubleValue] * [p.currency rateValueWithDate:p.date mainCurrency:currency]);
...
- (Payment*) lastPayment{
    ...
    NSPredicate *predicate = [NSPredicate predicateWithFormat:@"date <= %@ AND hidden == NO", [[NSDate today] dayNext] ];
...
- (Payment*) lastCategory{
    ...
    NSArray* arr = [[self.paymentsAsCategory allObjects] sortedArrayUsingDescriptors:sortDescriptors];
    NSPredicate *predicate = [NSPredicate predicateWithFormat:@"date <= %@ AND hidden == NO", [[NSDate today] dayNext]];
    arr = [arr filteredArrayUsingPredicate:predicate];
    if([arr count] > 0)
        return [arr objectAtIndex:0];
