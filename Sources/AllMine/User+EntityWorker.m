...
- (double) balanceForCurrentMonthFromLatestSalary:(int)type{
    NSDate *myDate = [[NSUserDefaults standardUserDefaults] objectForKey:@"paymentDate"];
    NSPredicate *predicate = [NSPredicate predicateWithFormat:@"finished == YES AND date >= %@ AND category.type == %d", myDate, type];
    NSNumber* num = [[self.payments filteredSetUsingPredicate:predicate] valueForKeyPath:@"@sum.sumMain"];
    return [num doubleValue];
}
...
- (NSArray*)reportAccountsWithItem:(ReportItem)item
    ...
    for (Account* acc in self.accounts) {
        accountSum = [acc balanceFromDate:dtFrom toDate:dtTo];
        if(acc.type == [NSNumber numberWithInt:AccTypeLoan]){
            accountSum*=-1;
        }
        if( acc.type == [NSNumber numberWithInt:AccTypeCardCredit ]){
            accountSum = (acc.balance - [acc.creditLimit doubleValue])*-1;
        }
        [result addObject:[self dictForReportAccount:acc sum:accountSum]];
    }
    return result;
}
...
- (NSArray*)reportTimeWithItem:(ReportItem)item
    ...
    switch (typeY) {
        ...
        case ReportTypeYOut:
            ...
            break;
        case ReportTypeYCapital:
            predicate = [NSPredicate predicateWithFormat:@"superpayment == nil AND removed == NO AND finished == YES AND user == %@ AND date >= %@ AND date < %@ AND hidden == YES", self, dtFrom, dtTo];
            break;
        default:
            break;
    }
    ...
    for(NSDate* dtLast = [self reportTimeInitDate:dtFrom reportItem:item];
        ...
        if (typeY != ReportTypeYCapital) {
            ...
            NSDate* accountDate = dtLast;
            
            NSSet* accountSet = usr.accounts;
            if (item == ReportItemMonth)
