...
- (Rate*) rateWithDate:(NSDate*)date{
    NSDate *startDate = [date dateWithoutTime];
    NSDate *finishDate = [startDate dateByAddingTimeInterval:60*60*24];
    
    NSPredicate *predicate = [NSPredicate predicateWithFormat:@"date >= %@ && date <= %@", startDate, finishDate];
    NSSet *st = [self.rates filteredSetUsingPredicate:predicate];
