...
- (void) viewWillAppear:(BOOL)animated{
    ...
    NSMutableArray *bankAccounts = [[NSMutableArray alloc] init];
    NSMutableArray *cashAccounts = [[NSMutableArray alloc] init];
    
    [bankAccounts addObjectsFromArray:[usr accountsByType:AccTypeBank]];
    [bankAccounts addObjectsFromArray:[usr accountsByType:AccTypeCardCredit]];
    
    [cashAccounts addObjectsFromArray:[usr accountsByType:AccTypeCash]];
    [cashAccounts addObjectsFromArray:[usr accountsByType:AccTypeLoan]];
    
    _accountsCash = cashAccounts;
    _accountsBank = bankAccounts;
    _accountsElectron = [usr accountsByType:AccTypeElecton];
    _accountsOther = [usr accountsByType:AccTypeOther];
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if(p != nil){
        ...
        lbInfo.textColor = [UIColor colorWithHex:kColorHexGray];
        lbInfo.text = [NSString stringWithFormat:@"%@ %+.2f", [p.date dateTitleDayMonthShort], [p.sumMain doubleValue]];
        UILabel *lbMoney = [[UILabel alloc] initWithFrame:CGRectMake(originX, 30, 140-originX, 20)];
        lbMoney.backgroundColor = [UIColor clearColor];
        lbMoney.font = [UIFont fontWithName:kFont1 size:11.0f];
        lbMoney.textColor = [UIColor colorWithHex:kColorHexBrown];
        lbMoney.text = [NSString stringWithFormat:@"%@" /*%+.2f"*/, [p.date dateTitleDayMonthShort]/*, [p.sumMain doubleValue]*/];
        ...
        lbInfo.font = [UIFont fontWithName:kFont1 size:11.0f];
        lbInfo.textColor = [UIColor colorWithHex:kColorHexGray];
        double sum = -[c.sumMain doubleValue];
        lbInfo.text = [NSString stringWithFormat:@"%@ %+.2f", [c.date dateTitleDayMonthShort], sum];
        UILabel *lbMoney = [[UILabel alloc] initWithFrame:CGRectMake(originX, 30, 140-originX, 20)];
        lbMoney.backgroundColor = [UIColor clearColor];
        lbMoney.font = [UIFont fontWithName:kFont1 size:11.0f];
        lbMoney.textColor = [UIColor colorWithHex:kColorHexBrown];
        lbMoney.text = [NSString stringWithFormat:@"%@" /*%+.2f"*/, [c.date dateTitleDayMonthShort]/*, [p.sumMain doubleValue]*/];
        if((lbInfo.text.length > 3) && ([[lbInfo.text substringFromIndex:lbInfo.text.length-3] isEqualToString:@".00"])){
            lbInfo.text = [lbInfo.text substringToIndex:lbInfo.text.length-3];
        }
        lbInfo.backgroundColor = [UIColor clearColor];
    ...
    double sum;
        ...
        if ([acc.type isEqualToNumber:[NSNumber numberWithInt:AccTypeCardCredit]])
        {
            NSNumber *sum1 = [NSNumber numberWithDouble:[acc creditLimitWithCurrency:acc.currency]];
            sum =  [sum1 doubleValue];
        }
        else
        {
            sum = [acc balance];
        ...
        if ([acc.type isEqualToNumber:[NSNumber numberWithInt:AccTypeCardCredit]])
        {
            lbSum.textColor = [UIColor colorWithHex:kColorHexGreen];
        }else if ([acc.type isEqualToNumber:[NSNumber numberWithInt:AccTypeLoan]] || [acc.group isEqualToNumber:[NSNumber numberWithInt:AccGroupDebts]])
        {
            lbSum.textColor = [UIColor colorWithHex:kColorHexOrange];
        }
