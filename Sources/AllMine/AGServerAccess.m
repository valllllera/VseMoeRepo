...
#define kServAccountCreditLimit @"creditlimit"
...
#define kServAccountTypeLoan   @"LOAN"
#define kServAccountTypeElectron   @"ELECTRON"
...
-(void)synchCatigoriesWithToken:(NSString*) token withContext:(NSManagedObjectContext*)context withUser:(User*)usr
{
    NSLog(@"1");
...
-(void)synchCurrenciesWithToken:(NSString*) token withContext:(NSManagedObjectContext*)context withUser:(User*)usr
{
...
-(void)synchAccountsWithToken:(NSString*) token withContext:(NSManagedObjectContext*)context withUser:(User*)usr
{
    ...
    for (NSDictionary* d in servAccountListAll) {
        ...
        int servCreditLimit = [[d objectForKey:kServAccountCreditLimit] intValue];
       ...
       if ([servTypeStr isEqualToString:kServAccountTypeBank]) {
            ...
            servType = AccTypeOther;
        }else if ([servTypeStr isEqualToString:kServAccountTypeLoan]){
            servType = AccTypeLoan;
        }else if ([servTypeStr isEqualToString:kServAccountTypeElectron]){
            servType = AccTypeElecton;
        }
        ...
        if(acc == nil){
            ...
            acc.creditLimit = [NSNumber numberWithInt:servCreditLimit ];
            //                [dbw saveManagedContext];
        }else{
            // ------ ACCOUNT - modify
            ...
            if(modLocal < modRemote){
                ...
                if (acc.creditLimit.intValue != servCreditLimit){
                    [self accountUpdateWithToken:token
                                       idAccount:acc.idAccount
                                        property:kServAccountCreditLimit
                                        valueInt:acc.creditLimit.intValue];
                }
                if (acc.type.intValue != servType) {
                    ...
                    switch (acc.type.intValue) {
                        ...
                        case AccTypeCardCredit:
                            ...
                            break;
                        case AccTypeElecton:
                            typeStr = kServAccountTypeElectron;
                            break;
                        case AccTypeLoan:
                            typeStr = kServAccountTypeLoan;
                            break;
                        default:
...
-(void)synchPaymentWithToken:(NSString*) token withContext:(NSManagedObjectContext*)context withUser:(User*)usr
{
...
-(void)synchTemplatesWithToken:(NSString*) token withContext:(NSManagedObjectContext*)context withUser:(User*)usr
{
...
//    usr = [User userWithLogin:usr.login context:context];
    ...
    //    return;
        ...
        // SYNC CURRENCIES
        {
            ...
            [self synchCurrenciesWithToken:token withContext:context withUser:usr];
                    }
        //-------------------------------------
        // SYNC RATES
        {
            AGLog(@"\r\n----------------------------------------------------------------------------");
            [self synchRatesWithToken:token withContext:context];
                    }
        //-------------------------------------
        // SYNC CATEGORIES
        {
            ...
            [self synchCatigoriesWithToken:token withContext:context withUser:usr];
                    }
        ...
        // SYNC ACCOUNTS
        {
            ...
            [self synchAccountsWithToken:token withContext:context withUser:usr];
                   }
        ...
        // SYNC TEMPLATES
        {
            ...
            [self synchTemplatesWithToken:token withContext:context withUser:usr];
                   }
        ...
        // SYNC PAYMENTS
        {
            ...
            [self synchPaymentWithToken:token withContext:context withUser:usr];
...
- (void) paymentDeleteWithToken:(NSString*)token
    ...
    [self sendRequestWithAction:@"/transaction/delete" withParams:[NSDictionary dictionaryWithObjectsAndKeys:
                                                                   token,     kServToken,
                                                                   idPayment, kServPaymentId, nil] withSuccess:nil withFailure:nil];
...
- (void) paymentUpdateWithToken:(NSString*)token
    ...
    [self sendRequestWithAction:[@"/transaction/update/" stringByAppendingString:property] withParams:[NSDictionary dictionaryWithObjectsAndKeys:
                                                                                                       token,     kServToken,
                                                                                                       idPayment, kServPaymentId,
                                                                                                       value,     kServValue, nil] withSuccess:nil withFailure:nil];}
- (void) paymentUpdateWithToken:(NSString*)token
                      idPayment:(NSNumber*)idPayment
                       property:(NSString*)property
                       valueInt:(int)valueInt{
...
- (void) accountDeleteWithToken:(NSString*)token
    ...
    [self sendRequestWithAction:@"/account/delete" withParams:[NSDictionary dictionaryWithObjectsAndKeys:
                                                               token,     kServToken,
                                                               idAccount, kServAccountId, nil] withSuccess:nil withFailure:nil];
}
...
- (void) accountUpdateWithToken:(NSString*)token
  ...
  /*  NSString* tokenReq = [RequestManager sharedRequest].token;
    [self sendRequestWithAction:[@"/account/update/" stringByAppendingString:property] withParams:[NSDictionary dictionaryWithObjectsAndKeys:
                                                                                                   tokenReq,     kServToken,
                                                                                                   idAccount, kServAccountId,
                                                                                                   value,     kServValue, nil] withSuccess:nil withFailure:nil];*/
}
...
- (NSString*) userToken:(User*)usr{
    ...
    NSLog(@"USET TOKEN %@",[[dict objectForKey:kServData] objectForKey:kServToken]);
...
-(void) settingsSetFromServerForUser:(User*)usr{
    ...
    NSString* token = [RequestManager sharedRequest].token;
    [self sendRequestWithAction:@"/settings/list" withParams:[NSDictionary dictionaryWithObjectsAndKeys:
                                                              token,     kServToken,
                                                              @"iphone_",  @"option",
                                                              @"1",  @"json",
                                                              nil] withSuccess:^(id json) {
        
        usr.syncAuto = [NSNumber numberWithBool:[[json objectForKey:kServSettingsSyncAuto] intValue] == 1 ? YES : NO];
        usr.syncViaWiFiOnly = [NSNumber numberWithBool:[[json objectForKey:kServSettingsSyncViaWiFIOnly] intValue] == 1 ? YES : NO];
...
- (id) sendRequest:(NSURLRequest*) aRequest {
	...
	NSData* data = [NSURLConnection sendSynchronousRequest:aRequest
                                         returningResponse:nil
                                                     error:&error];
    /*[NSURLConnection sendAsynchronousRequest:aRequest queue:<#(NSOperationQueue *)#> completionHandler:<#^(NSURLResponse *response, NSData *data, NSError *connectionError)handler#>]*/
...
//        [AGTools showAlertOkWithTitle:NSLocalizedString(@"Error", @"") message:error.localizedDescription];
	...
	if (data){
       ...
       // [RequestManager sharedRequest].token = [[response objectForKey:@"data"] objectForKey:@"token"];
        if (error != nil) {
