...
@property(nonatomic, retain) UITextView* tvComment;
...
@property(nonatomic, assign) BOOL isConfiguredCard;
@property(nonatomic, assign) BOOL isConfiguredNumber;
...
- (void)viewDidLoad
{
    ...
    if(_account){
        ...
        switch (_titleEC) {
            case 1:
                self.navigationItem.title = NSLocalizedString(@"TitleCash", @"");
                break;
            case 2:
                self.navigationItem.title = NSLocalizedString(@"TitleCash2", @"");
                break;
            case 3:
                self.navigationItem.title = NSLocalizedString(@"TitleLoan", @"");
                break;
            case 4:
                self.navigationItem.title = NSLocalizedString(@"TitleCard", @"");
                break;
            case 5:
                self.navigationItem.title = NSLocalizedString(@"TitleCredit", @"");
                break;
            case 6:
                self.navigationItem.title = NSLocalizedString(@"TitleBank", @"");
                break;
            case 7:
                self.navigationItem.title = NSLocalizedString(@"TitleElectron", @"");
                break;
            case 8:
                self.navigationItem.title = NSLocalizedString(@"TitleOther", @"");
                break;
            default:
                break;
        }
    ...
    if(_account){
        ...
        _bank = _account.bank;
        _creditLimit = [_account creditLimitWithCurrency:_currency];
...
//        btnReport.backgroundColor = [UIColor colorWithHex:kColorHexGreen];
    ...
    CGRect frameView = CGRectMake(5, 10, 300, 40);
    float sizeTxt = 17.0f;
    ...
    _tvComment = [[UITextView alloc] initWithFrame:frameView];
    _tvComment.font = [UIFont fontWithName:kFont1 size:sizeTxt];
    _tvComment.delegate = self;
    _tvComment.text = NSLocalizedString(@"AccountEditComment", @"");
    _tvComment.textColor = [UIColor colorWithHex:kColorHexGray];
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    [self tableViewGrouped:_tvItems
              configureCell:cell
...
- (void) tableViewGrouped:(UITableView*)tableView
    ...
    if (indexPath.row == 0) {
        background = [[UIImageView alloc] initWithImage:
                      [UIImage imageNamed:@"cell_account-top.png"]];
        ...
        background = [[UIImageView alloc] initWithImage:
                      [UIImage imageNamed:@"cell_account-end.png"]];
        backgroundHighlighted = [[UIImageView alloc] initWithImage:
                                 [UIImage imageNamed:@"cell-bottom-highlighted"]];
    } else {
        background = [[UIImageView alloc] initWithImage:
                      [UIImage imageNamed:@"cell_account-middle.png"]];
        backgroundHighlighted = [[UIImageView alloc] initWithImage:
...
- (void) configureCellBalance:(UITableViewCell*)cell{
    ...
    if(self.accountType == AccTypeCardCredit || self.accountType == AccTypeLoan || self.group == AccGroupDebts){
        [self.lbBalance setTextFromNumber:_balance
                                asInteger:NO
                                withColor:[UIColor colorWithHex:kColorHexOrange2]];
        if(!((self.account == nil) && (self.balance == 0) && !_isEnteringBalance) && self.accountType == AccTypeCardCredit)
            self.lbBalance.font = [UIFont fontWithName:kFont2 size:24.0f];
    }else{
        [self.lbBalance setTextFromNumber:_balance
                                asInteger:NO
                                withColor:nil];
    }
    if((self.account == nil) && (self.balance == 0) && !_isEnteringBalance){
    ...
    self.lbBalance.textAlignment = NSTextAlignmentLeft;
...
- (void) configureCellCreditLimit:(UITableViewCell*)cell{
    ...
    if((self.account == nil) && (self.creditLimit == 0)){
        ...
        self.lbCreditLimit.font = [UIFont fontWithName:kFont2 size:24.0f];
    }
    self.lbCreditLimit.textAlignment = NSTextAlignmentLeft;
...
- (void) systemKeyboardWillShow{
    ...
    if (_tfBank.isFirstResponder) {
        ...
        textField = _tfCard;
    }else if (_tvComment.isFirstResponder){
        textView = _tvComment;
...
-(void)textFieldDidBeginEditing:(UITextField *)textField{
    if (textField == _tfNumber){
        _tfNumber.placeholder =@"";
        _tfNumber.font = [UIFont fontWithName:kFont1 size:20.0f];
    }
    if (textField == _tfCard){
        _tfCard.placeholder =@"";
        _tfCard.font = [UIFont fontWithName:kFont1 size:20.0f];
    }

}
-(void)textViewDidBeginEditing:(UITextView *)textView{
    if (textView == _tvComment) {
        if([_tvComment.text isEqual:NSLocalizedString(@"AccountEditComment", @"")]){
            _tvComment.text = @"";
            _tvComment.textColor = [UIColor colorWithHex:kColorHexBlack];
        }
    }
}
...
-(void)keyboardNumberPressed:(NSString *)number
    ...
    if(self.accountType == AccTypeCardCredit || self.accountType == AccTypeLoan || self.group == AccGroupDebts){
        [lb setTextFromNumber:result
                                ...
                                withColor:[UIColor colorWithHex:kColorHexOrange2]];
        if(self.accountType == AccTypeCardCredit)
            lb.font = [UIFont fontWithName:kFont2 size:24.0f];
    }else{
        [lb setTextFromNumber:result
                                asInteger:NO
                                withColor:nil];
