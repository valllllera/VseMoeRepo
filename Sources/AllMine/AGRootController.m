...
-(void)viewWillAppear:(BOOL)animated
{
    ...
    if ((kUser).syncAuto.boolValue == YES)
    {
...
//    _currentUser = [User userWithLogin:@"user"];
 ...
 //---
    ...
    UILabel *lb_load=[[UILabel alloc] initWithFrame:CGRectMake(0, 30, 100, 50)];
    ...
    NSLog(@"bool %d",(kUser).syncAuto.boolValue);
...
- (void) synchronize{
    ...
    if (sync)
    {
        @try {
            [self blockWindowWithActivityIndicator:YES];
            //[[RequestManager sharedRequest] synchronizeDataForUser:(kUser)];
            [[RequestManager sharedRequest] synchronizeDataForUser:(kUser) withSuccess:^{
                NSLog(@"ok sycn");
                [self blockWindowWithActivityIndicator:NO];
                self.isSyncActive = NO;
   ...
   // [self blockWindowWithActivityIndicator:NO];
    ...
    if (sync) {
        ...
        @try {
            ...
            NSString *token =  [sa userToken:kUser];
            
            @try {
                [sa synchronizeDataForUser:(kUser) withToken:token];
