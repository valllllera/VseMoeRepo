...
- (void)viewDidLoad
{
    ...
    if (_payment) {        
        ...
        if ((self.category) && (self.category.type.intValue == CatTypeExpense) && self.sum!=0) {
            self.sum *= (-1);
        }
...
//                      forState:UIControlStateHighlighted];
 ...
 //---TextView---
    ...
    _lbSumResult.font = [UIFont fontWithName:kFont2 size:50.0f];
...
- (void) save{
    ...
    if (self.sum >= 0) {
        ...
        if(self.category==nil && self.sum!=0){
            _sum *= (-1);
        }        
        ...
        if (_rateValue > 0.0f)
        {
            ...
            if(!isnan([_currency rateValueWithDate:_payment.date mainCurrency:usr.currencyMain])) {   sumMain = _sum * [_currency rateValueWithDate:_payment.date mainCurrency:usr.currencyMain];
            }else sumMain = _sum;
        }
        ...
        if([_payment.category.title isEqual: @"Зарплата"]){
            [[NSUserDefaults standardUserDefaults] setObject:_payment.date forKey:@"paymentDate"];
...
-(void)keyboardNumberPressed:(NSString *)number
    ...
    if(!_kbCtl.operationString && !_keyboardPrintToLbOps){
        _lbSumResult.text = operationString;
    }
...
-(void)setSum:(double)sum
{
    ...
    if(self.category==nil && self.sum<0 && self.payment != nil){
        _sum *= (-1);
    }
    
    if([self.lbOps.text isEqualToString:@""]){
        if(_kbCtl.operationString && _sum>0)
        {
            [_lbSumResult setText:_kbCtl.operationString];
            [_lbSumResult setTextColor:[UIColor colorWithHex:kColorHexPaymentResult]];
