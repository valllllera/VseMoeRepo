...
- (void)viewDidLoad
{
    ...
    self.navigationItem.rightBarButtonItem = [AGTools navigationBarButtonItemWithImageNamed:@"button-transaction" target:self action:@selector(toPaymentCommon)];
    ...
    _lbSum.font = [UIFont fontWithName:kFont2 size:48.0f];
...
//    self.currency = _currency;
    ...
    for (int i = 0; i < num; i++) {
        ...
        UILabel* lb = [[UILabel alloc] initWithFrame:CGRectMake(0, 23, width, 23)];
        if (isIphoneRetina4) {
            CGRect rect=CGRectMake(0, 50, width, 21);
            lb.frame=rect;
        }
        lb.textAlignment = UITextAlignmentCenter;
        lb.font = [UIFont fontWithName:kFont1 size:9.0];
        lb.textColor = [UIColor colorWithHex:kColorHexDarkGray];
