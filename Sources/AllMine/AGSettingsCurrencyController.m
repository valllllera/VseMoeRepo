...
-(UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section{
    switch (section) {
        case 1:
            return [self tableViewGroupedHeaderViewWithTitle:[NSLocalizedString(@"SettingsCurrencyAdditional", @"") uppercaseString]
...
//    UITableViewCell* cell = [tableView cellForRowAtIndexPath:indexPath];
    if(indexPath.row == 0){
        ...
        ctl.mainCurrency = _mainCurrency;
        ctl.isAccount = NO;
        if(indexPath.section==0)
            ctl.isMainCurrencyEdit = YES;
        else
...
-(void)currencyListController:(AGCurrencyListController *)controller
    deleteCurrency:(Currency *)currency{
    
    User* usr = (kUser);
    [usr removeCurrenciesAdditionalObject:currency];
    [[AGDBWorker sharedWorker] saveManagedContext];
    [_additionalCurrencies removeObject:currency];
}
